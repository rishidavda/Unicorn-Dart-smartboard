...
| `porttest.js`, `freshedge.js`, `offcheck.js` | Sticky ports for two boards on one PC (a hand-set port survives the upgrade's `PORT=8080` and legacy homes never record 8080 as their origin, a moved folder keeps its port, a copied folder takes the next one even when started first, Find boards from the copy's console leaves the original's port, identity and board alone); fresh-start settings, launcher-gone pause, one fresh start a day even after a port change - also when the stagger crosses midnight (that check runs the hub under `faketime`; skipped without it), the card label after the day's restart |
