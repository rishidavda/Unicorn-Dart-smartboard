...
async function probe(label, env, expect) {
  ...
  const c = spawn('node', [process.env.SERVER_JS || require('path').join(HERE, '..', 'server', 'server.js')],
    { env: { ...process.env, PORT: '8893', DARTS_DATA: `${SP}/hubOff`, ...env }, stdio: 'ignore' });
  await wait(2500);
  const s = io('http://127.0.0.1:8893');
...
(async () => {
  const r = [
    ...
    await probe('custom time honoured', { WINCHESTER_SUPERVISED: '1', DAILY_RESTART: '06:30' }, '06:33'),
